...
__interrupt static void r_uart0_interrupt_receive(void)
{
    ...
    if(bflg_test_mode == 1)
    {
        guc_test_usb_buf = rx_data;
    }
    else
    {
        com_rx_int_usb(rx_data);    //接收usb板的数据
    }
}
...
__interrupt static void r_uart1_interrupt_receive(void)
{
    ...
    if(bflg_test_mode == 1)
    {
        test_mode_com_rx_int(rx_data);
    }
    else
    {
        com_rx_int(rx_data); // 接收主板发送的信息
    }
}
